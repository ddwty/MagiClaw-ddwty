...
func resizePixelBuffer(_ pixelBuffer: CVPixelBuffer, width: Int, height: Int) -> CVPixelBuffer? {
    ...
    CVPixelBufferCreate(kCFAllocatorDefault, width, height, CVPixelBufferGetPixelFormatType(pixelBuffer), attrs, &newPixelBuffer)
    
    guard let dstPixelBuffer = newPixelBuffer else { return nil }
    
    CVPixelBufferLockBaseAddress(dstPixelBuffer, [])
    defer { CVPixelBufferUnlockBaseAddress(dstPixelBuffer, []) }
    
    guard let dstBaseAddress = CVPixelBufferGetBaseAddress(dstPixelBuffer) else { return nil }
    ...
    var dstBuffer = vImage_Buffer(data: dstBaseAddress,
                                  height: vImagePixelCount(height),
                                  width: vImagePixelCount(width),
